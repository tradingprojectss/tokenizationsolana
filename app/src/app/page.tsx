...
export default function HomePage() {
  ...
  return (
    <div>
          ...
          <div className="max-w-3xl">
            ...
            <div className="mt-6 sm:mt-8 flex flex-wrap items-center gap-3 sm:gap-4 animate-slide-up" style={{ animationDelay: "0.3s" }}>
              <div className="flex items-center gap-3 px-5 py-3.5 rounded-2xl glass border border-blue-500/10 hover:border-blue-500/30 transition-colors">
                <div className="text-2xl sm:text-3xl font-black text-gradient">~52%</div>
      ...
      {/* ── HOW THE MODEL WORKS ──────────────────────────────────────── */}
      <section className="border-y border-white/5 bg-[#0d1117] py-14 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-14">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass text-xs text-blue-400 font-bold mb-4 border border-blue-500/20">
              <BadgeCheck className="h-3.5 w-3.5" /> Как работает модель
            </div>
            <h2 className="text-2xl sm:text-4xl font-black text-white">{t("how.title")}</h2>
            <p className="mt-3 text-slate-400 font-medium max-w-xl mx-auto text-sm sm:text-base">
              Покупатель живёт в квартире сразу. Инвесторы получают доход. Все выигрывают.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-6 mb-10">
            {[
              { step: "01", icon: Building2, color: "bg-blue-600", glow: "group-hover:shadow-blue-500/20", tag: "Токенизация", title: t("how.step1.title"), desc: t("how.step1.desc") },
              { step: "02", icon: Users,     color: "bg-violet-600", glow: "group-hover:shadow-violet-500/20", tag: "Краудфандинг", title: t("how.step2.title"), desc: t("how.step2.desc") },
              { step: "03", icon: TrendingUp, color: "bg-emerald-600", glow: "group-hover:shadow-emerald-500/20", tag: "Выкуп", title: t("how.step3.title"), desc: t("how.step3.desc") },
            ].map((item, i) => (
              <div key={item.step}
                className={`group relative rounded-2xl bg-[#111827] border border-white/5 p-6 sm:p-7 hover:border-white/10 hover:shadow-xl ${item.glow} transition-all duration-300`}
                style={{ animationDelay: `${i * 0.15}s` }}>
                <div className="text-6xl font-black text-white/[0.03] absolute top-3 right-4">{item.step}</div>
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-11 h-11 rounded-xl ${item.color} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
                    <item.icon className="h-5 w-5 text-white" />
                  </div>
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-white/5 text-slate-500 border border-white/10">{item.tag}</span>
                </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
            {[
              { value: "$8",  label: "цена токена",    sub: "SPL USDC · Solana",          color: "text-blue-400",   border: "border-blue-500/20",   bg: "bg-blue-500/5" },
              { value: "8.5%", label: "доходность",    sub: "годовых для инвесторов",      color: "text-emerald-400", border: "border-emerald-500/20", bg: "bg-emerald-500/5" },
              { value: "5–7", label: "лет до 100%",    sub: "без ипотеки",                 color: "text-purple-400", border: "border-purple-500/20",  bg: "bg-purple-500/5" },
              { value: "~52%", label: "экономия",      sub: "vs банковская ипотека 18%",   color: "text-amber-400",  border: "border-amber-500/20",   bg: "bg-amber-500/5" },
            ].map((s) => (
              <div key={s.label} className={`text-center p-4 sm:p-5 rounded-2xl ${s.bg} border ${s.border} hover:scale-[1.02] transition-transform duration-200`}>
                <div className={`text-2xl sm:text-3xl font-black ${s.color}`}>{s.value}</div>
                <div className="text-xs sm:text-sm text-slate-300 mt-1 font-semibold">{s.label}</div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass text-xs text-purple-400 font-bold mb-4 border border-purple-500/20">
              <Sparkles className="h-3.5 w-3.5" /> Реальный сценарий
            </div>
            <h2 className="text-2xl sm:text-4xl font-black text-white">История Асель</h2>
            <p className="mt-3 text-slate-400 font-medium">Данные с krisha.kz · ЖК Namys, Астана</p>
          </div>

          <div className="rounded-2xl sm:rounded-3xl overflow-hidden border border-white/10 bg-[#0d1117]">
            <div className="p-6 sm:p-8">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-black text-xl shadow-lg shadow-blue-500/20 flex-shrink-0">А</div>
                <div>
                  <div className="font-bold text-white text-lg">Асель, 26 лет</div>
                  <div className="text-sm text-slate-500 font-medium">Астана · зарплата 500 000 ₸/мес · мечтает о своей квартире</div>
                </div>
              ...
              <div className="space-y-4 text-slate-300 leading-relaxed font-medium text-sm sm:text-base mb-6">
                <div className="flex gap-3">
                  <div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold text-white">1</div>
                  <p>Нашла 1-комнатную за <span className="text-white font-bold">$46,400</span> в ЖК Namys. Накопила <span className="text-white font-bold">$10,000</span> — не хватает $36,400.</p>
                </div>
                <div className="flex gap-3">
                  <div className="w-6 h-6 rounded-full bg-violet-600 flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold text-white">2</div>
                  <p>Создаёт кампанию: <span className="text-blue-400 font-bold">5 800 токенов × $8 = $46,400</span>. Покупает 1 250 токенов (21.5%). <span className="text-purple-400 font-bold">85 инвесторов</span> закрывают остаток.</p>
                </div>
                <div className="flex gap-3">
                  <div className="w-6 h-6 rounded-full bg-emerald-600 flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold text-white">3</div>
                  <p>Заселяется сразу. Платит <span className="text-white font-bold">$350/мес</span> как аренду инвесторам + ежемесячно выкупает токены. <span className="text-emerald-400 font-bold">100% владение за 5–6 лет.</span></p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-2xl bg-red-500/5 border border-red-500/20 p-5 text-center">
                  ...
                  <div className="text-xs text-slate-500 mt-1">итого выплат</div>
                  <div className="text-xs text-red-400 font-bold mt-0.5">переплата $99K</div>
                </div>
                <div className="rounded-2xl bg-emerald-500/5 border border-emerald-500/20 p-5 text-center relative overflow-hidden">
                  <div className="absolute top-2 right-2"><Sparkles className="h-4 w-4 text-emerald-500/40" /></div>
                  ...
                  <div className="text-xs text-slate-500 mt-1">итого выплат</div>
                  <div className="text-xs text-emerald-400 font-bold mt-0.5">переплата всего $11K</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-16 sm:py-24 bg-[#0d1117]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass text-xs text-blue-400 font-bold mb-4 border border-blue-500/20">
              <Calculator className="h-3.5 w-3.5" /> Посчитай сам
            </div>
