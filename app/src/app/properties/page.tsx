    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-6 sm:mb-8">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder={t("props.search")} value={search} onChange={(e) => setSearch(e.target.value)}
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          ...
          {[
            ...
            <button key={s.key} onClick={() => setStatusFilter(s.key)}
              className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${statusFilter === s.key ? "bg-blue-600 text-white" : "glass border border-white/5 text-slate-400 hover:text-white"}`}>
              {s.label}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-20"><p className="text-slate-500 font-medium">{t("props.not_found")}</p></div>
      )}
    </div>
  );
}
